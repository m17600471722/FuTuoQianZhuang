   ...
   <div class="logIn-loading" v-if="loading">
    ...
    <div class="exten">
      <div class="exten_list">
        ...
        <div class="extenli">
          <span>展期还款日：</span>
          <span class="textval">{{result.extensionShouldRepayDate}}</span>
        </div>
        <div class="extenli">
          <span>到期应还(元)：</span>
          <span class="textval">{{parseFloat(result.oldShouldRepayTotal).toFixed(2)}}</span>
        </div>
        <div class="extenli">
          <span>展期费用(元)：</span>
      ...
      <div class="repaybtn" @click="repayClick">
...
import { getCardSms,repayment,saveCard,getCardInfo,repayInfo,repaymentSendSms } from "../../api/app.api";
